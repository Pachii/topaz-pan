...
void VocalWidenerProcessor::parameterChanged (const juce::String& parameterID, float newValue)
{
    if (isLinkingPan.exchange(true)) return;
    
    bool linked = linkPanParam != nullptr && linkPanParam->load(std::memory_order_relaxed) > 0.5f;
...
juce::AudioProcessorValueTreeState::ParameterLayout VocalWidenerProcessor::createParameterLayout()
{
    std::vector<std::unique_ptr<juce::RangedAudioParameter>> params;

    params.push_back(std::make_unique<juce::AudioParameterFloat>(juce::ParameterID("offsetTime", 1), "offset time", 0.0f, 50.0f, 10.0f));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(juce::ParameterID("leftPan", 1), "left pan", -1.0f, 1.0f, -1.0f));
    ...
    params.push_back(std::make_unique<juce::AudioParameterFloat>(juce::ParameterID("haasCompAmt", 1), "haas comp amount", 
        juce::NormalisableRange<float>(0.0f, 200.0f, 1.0f), 100.0f, 
        juce::AudioParameterFloatAttributes().withStringFromValueFunction([](float v, int) { return juce::String(juce::roundToInt(v)); })));

    return { params.begin(), params.end() };
}

bool VocalWidenerProcessor::isBusesLayoutSupported (const BusesLayout& layouts) const
{
...
void VocalWidenerProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    juce::ignoreUnused (samplesPerBlock);

    isStereoLayout = (getTotalNumInputChannels() == 2 && getTotalNumOutputChannels() == 2);

    delayLeft.prepare(sampleRate, 100.0f);
    delayRight.prepare(sampleRate, 100.0f);
    pitchLeft.prepare(sampleRate, 30.0f);
    pitchRight.prepare(sampleRate, 30.0f);
    
    // In Centered Timing mode, we have a fixed structural latency (half of max offset)
    // If you need exact DAW alignment, report latency here. We are doing 0 to keep it simple as requested.
    setLatencySamples(0); 
}

void VocalWidenerProcessor::releaseResources() {}

void VocalWidenerProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    juce::ignoreUnused (midiMessages);
    juce::ScopedNoDenormals noDenormals;

    // Fast bypass and Non-Stereo Layout safe exit
    if (!isStereoLayout || bypassParam->load(std::memory_order_relaxed) > 0.5f) return;
    ...
    float gainLinear = juce::Decibels::decibelsToGain(outputGainParam->load(std::memory_order_relaxed));
    ...
    float haasAmtNorm = haasCompAmtParam->load(std::memory_order_relaxed) / 100.0f;
    ...
    if (haasEnable)
    {
        ...
        if (deltaMs > 0.1f) // Only correct if noticeably different
        {
            ...
            if (delayTLeftMs < delayTRightMs)
            {
                ...
                if (std::abs(leftPan - rightPan) < 0.05f) earlierPath = -1.0f;
                else earlierPath = (leftPan < rightPan) ? 0.0f : 1.0f;
            }
            else
            {
                ...
                if (std::abs(leftPan - rightPan) < 0.05f) earlierPath = -1.0f;
                else earlierPath = (rightPan < leftPan) ? 0.0f : 1.0f;
            }
        }
    }
    ...
    float leftPL = std::cos((leftPan + 1.0f) * juce::MathConstants<float>::pi * 0.25f);
    float leftPR = std::sin((leftPan + 1.0f) * juce::MathConstants<float>::pi * 0.25f);
    
    float rightPL = std::cos((rightPan + 1.0f) * juce::MathConstants<float>::pi * 0.25f);
    float rightPR = std::sin((rightPan + 1.0f) * juce::MathConstants<float>::pi * 0.25f);
...
void VocalWidenerProcessor::setStateInformation (const void* data, int sizeInBytes)
{
